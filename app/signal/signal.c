...
const char * Signal_GetName(signal_id_t id)
{
    ...
    static const char * const k_names[SIG_MAX] = {
        ...
        /* NOTE: SIG_CAN_<DbcSignal> entries generated by
         * tools/dbc_parse.py (app/drv_api/can/can_db_ipk_gen.h)
         * are intentionally not duplicated in this hand-maintained
         * table; for those ids Signal_GetName() falls through to
         * the "<can-signal>" return at the bottom of the function.
         * Regenerating this table from the autogen header would be
         * possible but offers no functional gain (the logs are
         * already readable via SIG_CAN_<Name> in source). */
    };
     ...
     * any id at or after SIG_CAN_RX_TIMEOUT_MAP_HI2 falls into the
     * autogen CAN block.  Return a stable placeholder so logs remain
     * readable without claiming a precise offset that may shift with
     * future DBC growth. */
