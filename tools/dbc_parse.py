...
class Message:
    ...
    cycle_time: int = 0            # BA_ "GenMsgCycleTime"  BO_ <id> <ms>;  0 if absent
    send_type:  int = 0            # BA_ "GenMsgSendType"   BO_ <id> <v>;   0=cycle 1=event
...
def parse_dbc(path: str) -> List[Message]:
    ...
    # Second pass: pick up BO_-scoped attributes.
    #   BA_ "GenMsgCycleTime" BO_ <can_id> <ms>;
    ...
    with open(path, "r", encoding="utf-8", errors="replace") as f:
        for line in f:
            mc = _re_ba_bo_cycle.match(line)
            if mc:
                m = by_id.get(int(mc.group(1)))
                if m is not None:
                    m.cycle_time = int(mc.group(2))
                continue
            ms = _re_ba_bo_send.match(line)
            if ms:
...
def emit_source(node, msgs, enum_names):
    ...
    L.append("};")
    L.append("")
    L.append(f"/* CAN_DB_{node.upper()}_RX_COUNT / _TX_COUNT live in can_db_{node.lower()}_gen.h */")
...
def _default_cycle_ms(msg):
    ...
    Order of preference:
      1. msg.cycle_time (parsed from DBC BA_ "GenMsgCycleTime")
      2. Heuristic by msg name for TX-only messages that lack a DBC
         cycle_time (STS / SettingRequest = 100 ms, fuel / odo /
         datetime / nwm = 500 ms, other = 1000 ms)
      3. 0 when the message is event-driven (GenMsgSendType == 1)
         and has no DBC cycle_time
    """
    if msg.cycle_time > 0:
        return msg.cycle_time
    if msg.send_type == 1:
        # Event-driven; no fixed cycle in DBC.
        return 0
    n = msg.name.lower()
...
def emit_tx_cycle_table(selected, node):
    """Emit g_can_tx_cycle_table[CAN_DB_IPK_TX_COUNT] body.

    Layout: TX rows only, indexed by TX slot 0..TX_COUNT-1.
    Readers (can_tx.c prv_build_tx_table) feed the same TX
    slot index used for s_tx.payloads[].cycle_ms.  RX rows
    never appear here -- their cycle is meaningless.
    """
    out = [
    ...
    for m in selected:
        ...
        out.append(f"    /* {slot:>3} {m.name:<32} (TX) */ {cyc}u,")
        slot += 1
    out.append("};")
    return "\n".join(out) + "\n"
...
def emit_rx_timeout_table(selected, node):
    """Emit g_can_rx_timeout_table[CAN_DB_IPK_RX_COUNT] body.

    Project rule (set by C02-B2 cluster network team):
      cycle <  50 ms : fixed 250 ms (fast loop, allow jitter)
      cycle >= 50 ms : cycle * 5 (5 missed cycles = timeout)
      event-driven   : 5000 ms fallback (DBC BA_ "GenMsgSendType" == 1)
      no cycle       : 0 (do not monitor)

    Layout: RX rows only, indexed by RX slot 0..RX_COUNT-1.
    Readers (can_rx.c prv_check_timeouts) feed the same RX
    slot index used for s_rx.raw[].  TX rows never appear
    here -- TX has no timeout.

    cycle comes from DBC BA_ "GenMsgCycleTime" BO_ <id> <ms>; when
    absent for an event-driven message we fall back to 5 s.
    """
    out = [
    ...
    for m in selected:
        if m.transmitter == node:
            continue
        cyc = m.cycle_time
        if cyc == 0 and m.send_type == 1:
            tmo = 5000
            note = "  /* event-driven, fallback 5s */"
        elif cyc == 0:
            tmo = 0
            note = "  /* no cycle, not monitored */"
        elif cyc < 50:
            tmo = 250
            note = f"  /* cycle={cyc}ms<50, fixed 250ms */"
        else:
            tmo = cyc * 5
            note = f"  /* cycle={cyc}ms*5 */"
        out.append(f"    /* {slot:>3} {m.name:<32} (RX) */ {tmo}u,{note}")
        slot += 1
    out.append("};")
    return "\n".join(out) + "\n"
